...
# @Time: 2021/7/16
# @File: test_networking_12_5_2_3.py
# @Author: Restrain
# @E-mail: qiangzhongzhi@huaqin.com
# @Desc: Android 11 CTS Networking_12_5_2_3 自动化测试脚本
...
class TestNetworkingSuggest3(unittest.TestCase):
    def open_test_networking(self):
        ...
        if d(text='OK').exists:
            d(text='OK').click()
            d.sleep(1)
        d(text='Network suggestion with SSID and post connection broadcast.').click()
        d.sleep(2)
        if d(text='OK').exists:
            d(text='OK').click()
            d.sleep(1)
