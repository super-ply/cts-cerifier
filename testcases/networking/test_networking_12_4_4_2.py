...
# @Time: 2021/7/19
# @File: test_networking_12_4_4_2.py
# @Author: Restrain
# @E-mail: qiangzhongzhi@huaqin.com
# @Desc: Android 11 CTS Networking_12_4_4_2 自动化测试脚本
...
class TestNetworkingGroup8(unittest.TestCase):
    def open_test_networking(self, d: uiautomator2.Device):
        ...
        if d(text='OK').exists:
            d(text='OK').click()
            d.sleep(1)
        d(text='Group Client With Config 2G Band Test').click()
        d.sleep(2)
        if d(text='OK').exists:
            d(text='OK').click()
            d.sleep(1)
        ...
        if d2(text='OK').exists:
            d2(text='OK').click()
            d2.sleep(1)
        d2(text='Group Owner With Config 2G Band Test').click()
        d2.sleep(2)
        if d2(text='OK').exists:
            d2(text='OK').click()
            d2.sleep(1)
    ...
    def check_result(self):
        self.sub_check_result('Join p2p group test (2.4G config)')
