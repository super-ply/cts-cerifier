...
# @Time: 2021/7/19
# @File: test_networking_12_4_1_2.py
# @Author: Restrain
# @E-mail: qiangzhongzhi@huaqin.com
# @Desc: Android 11 CTS Networking_12_4_1_2 自动化测试脚本
...
class TestNetworkingGroup2(unittest.TestCase):
    def open_test_networking(self, d: uiautomator2.Device):
        ...
        if d(text='OK').exists:
            d(text='OK').click()
            d.sleep(1)
        d(text='GO Negotiation Requester Test').click()
        d.sleep(2)
        if d(text='OK').exists:
            d(text='OK').click()
            d.sleep(1)
        d1.app_start(package_name=package, activity=activity, stop=True)
        if d1(text="Location permission").exists:
            d1.press('back')
            d1.sleep(3)
        d1(scrollable=True).scroll.to(text='Wi-Fi Direct Test')
        d1(text='Wi-Fi Direct Test').click()
        d1.sleep(2)
        if d1(text='OK').exists:
            d1(text='OK').click()
            d1.sleep(1)
        d1(text='GO Negotiation Responder Test').click()
        d1.sleep(2)
        if d1(text='OK').exists:
            d1(text='OK').click()
            d1.sleep(1)
