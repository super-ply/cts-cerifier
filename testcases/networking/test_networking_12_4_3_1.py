...
# @Time: 2021/7/19
# @File: test_networking_12_4_3_1.py
# @Author: Restrain
# @E-mail: qiangzhongzhi@huaqin.com
# @Desc: Android 11 CTS Networking_12_4_3_1 自动化测试脚本
...
class TestNetworkingGroup5(unittest.TestCase):
    def open_test_networking(self, d: uiautomator2.Device):
        ...
        if d(text='OK').exists:
            d(text='OK').click()
            d.sleep(1)
        d(text='Group Owner With Config Test').click()
        d.sleep(2)
        if d(text='OK').exists:
            d(text='OK').click()
            d.sleep(1)
        ...
        if d2(text='OK').exists:
            d2(text='OK').click()
            d2.sleep(1)
        d2(text='Group Client With Config Test').click()
        d2.sleep(2)
        if d2(text='OK').exists:
            d2(text='OK').click()
            d2.sleep(1)
