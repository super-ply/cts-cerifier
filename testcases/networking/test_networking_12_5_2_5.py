...
# @Time: 2021/7/16
# @File: test_networking_12_5_2_5.py
# @Author: Restrain
# @E-mail: qiangzhongzhi@huaqin.com
# @Desc: Android 11 CTS Networking_12_5_2_5 自动化测试脚本
...
class TestNetworkingSuggest5(unittest.TestCase):
    def open_test_networking(self):
        ...
        if d(text='OK').exists:
            d(text='OK').click()
            d.sleep(1)
        d(text='Network suggestion modification in place.').click()
        d.sleep(2)
        if d(text='OK').exists:
            d(text='OK').click()
            d.sleep(1)

    def open_test_wifi(self):
        """
        打开测试项
        :return:
        """
    ...
    def check_result(self):
        ...
        if d(resourceId="com.android.cts.verifier:id/pass_button").info.get('enabled'):
            ...
            d.sleep(2)
        if not d(text="Wi-Fi Test").exists:
            self.open_test_wifi()
            d.sleep(2)
        if d(resourceId="com.android.cts.verifier:id/pass_button").info.get('enabled'):
            d(resourceId="com.android.cts.verifier:id/pass_button").click()
