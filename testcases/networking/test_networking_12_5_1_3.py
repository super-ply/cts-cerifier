...
# @Time: 2021/7/16
# @File: test_networking_12_5_1_3.py
# @Author: Restrain
# @E-mail: qiangzhongzhi@huaqin.com
# @Desc: Android 11 CTS Networking_12_5_1_3 自动化测试脚本
...
class TestNetworkingRequest3(unittest.TestCase):
    def open_test_networking(self):
        ...
        if d(text='OK').exists:
            d(text='OK').click()
            d.sleep(1)
        d(text='Network Request with a specific network that is unavailable.').click()
        d.sleep(2)
        if d(text='OK').exists:
            d(text='OK').click()
            d.sleep(1)
    ...
    def check_result(self):
        ...
        d.sleep(5)
        # TODO 需要适配
        if not d(resourceId="com.coloros.wirelesssettings:id/oppo_preference").exists:
            d.press('back')
            d.sleep(2)
        else:
            d.press('back')
