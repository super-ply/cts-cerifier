...
# @Time: 2021/6/9
# @File: 13_8_shortcut_reset_rate_limiting_test.py
# @Author: Restrain
# @E-mail: qiangzhongzhi@huaqin.com
# @Desc: Android 11 CTS Notifications13.8 自动化测试脚本
...
class ShortcutResetRateLimitingTest(unittest.TestCase):
    def open_shortcut_reset_rate_limiting_test(self):
        """
        打开测试项
        :return:
        """
    ...
    def test_main(self):
        apk_path = os.path.join(os.path.join(rootPath, 'resource'), 'apk') + '\\NotificationBot.apk'
        os.system('adb install -r ' + apk_path)
        d.sleep(2)
        self.open_shortcut_reset_rate_limiting_test()
        self.check_result()
    ...
    def check_result(self):
        ...
        if pass_count == 2:
            ...
            d.sleep(1)
            d(resourceId="com.android.systemui:id/remote_input_text").set_text('messages')
            d.sleep(1)
        ...
        while not d(resourceId="com.android.cts.verifier:id/pass_button").info.get('enabled'):
            ...
            for test_item in test_items:
                ...
                if Compare.compare_template(src, temp_fail):
                    d(resourceId="com.android.cts.verifier:id/fail_button").click()
                    d.sleep(1)
                    raise AssertionError('测试失败,测试项：SHORTCUT_RESET_RATE_LIMITING_TEST')
